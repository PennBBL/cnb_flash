...
for (i in 1:length(textsAcc)) {
  ...
  # box plot
  boxLY <- ggplot(res, aes(x=factor(flash),y=residuals, color=factor(flash))) +
    geom_boxplot() + 
    labs(title = paste("Accuracy Difference in",textsAcc[i],"(Last year)")) +
    ylab("Residuals") + theme_bw() +
    scale_x_discrete("Flash",breaks=0:1,labels=c("Non-Flash", "Flash")) +
    scale_color_discrete("Flash",breaks=0:1,labels=c("Non-Flash", "Flash")) +
    ylim(-5,5)
...
We also looked at test accuracy and speed comparison across the different administration sites. This was done to see if some sites in particular showed big differences between Flash and non-Flash groups. At the same time, we checked for any sites that drove the mean up or down in each group.

```{r site diff acc, include=FALSE}
...
No site stands out in particular.

```{r GNG150 site dif, echo=FALSE}
GNG150_sitedif
```
...
LiBI and EXECTS show some differences between Flash and Non-Flash, but EXECTS does not have enough Flash data and LiBI might look different when combined with LIBI and libi_family. There are more negative Flash residuals (removing LIBI might help).

```{r KSPVRT D site dif, echo=FALSE}
KSPVRT_D_sitedif
```
...
RCNVPENN brings the Flash average down. EXECTS shows some differences between Flash and Non-Flash, but again, it does not have enough Flash data. (removing RCNVPENN might help) 

```{r PCET A site dif, echo=FALSE}
PCET_A_sitedif
```
...
LiBI looks like it shows some differences between Flash and non-Flash, but might look different when combined with LIBI and libi_family. There are more negative non-Flash residuals.

```{r SPCPTNL site dif, echo=FALSE}
SPCPTNL_sitedif
```



```{r site diff spe, include=FALSE}
...
MOTIVE shows some differences between Flash and non-Flash. 

```{r GNG150 site dif sp, echo=FALSE}
GNG150_sitedifSp
```
...
MOTIVE shows some differences that Flash is generally more positive non-Flash. 

```{r MPRACT site dif sp, echo=FALSE}
MPRACT_sitedifSp
```
...
EVOLPSY and LiBI show that Flash response times are a little bit higher than non-Flash. LIBI also looks like it might be driving up the Flash average.

```{r SPCPTNL site dif sp, echo=FALSE}
SPCPTNL_sitedifSp
```
