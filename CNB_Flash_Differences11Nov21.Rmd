...
The data used for this project comes from Tableau. It is comprised of over 10,000 participants' data on 34 tasks. For the purpose of comparing Flash and non-Flash administration on these tasks, the 17 tasks listed below (which had enough data for Flash and non-Flash) were used.
...
There seems to be a cluster of participants whose non-Flash scores are < -0.70 that look like outliers. This is what the correlation and ICC looks like upon removing these eight participants.

```{r, echo=FALSE}
newSPCPTNL_acc <- SPCPTNL_acc[order(SPCPTNL_acc$`Non-Flash`),]
newSPCPTNL_acc <- newSPCPTNL_acc[newSPCPTNL_acc$`Non-Flash` > (-0.7),]

names(newSPCPTNL_acc)[c(2,17)] <- c("Flash", "Non-Flash")
pairs.panels(newSPCPTNL_acc[,c(2,17)],lm=TRUE)

names(newSPCPTNL_acc)[c(2,17)] <- c("f_acc_res.1", "n_acc_res.1")
acc <- newSPCPTNL_acc
icc_fnf <- icc(acc[,grepl("acc_res",colnames(acc))],type="agreement",model="twoway")$value
try(icc_f12_1 <- icc(acc[,grepl("f_acc_res",colnames(acc)) | grepl("f_t2newscore1",colnames(acc))],type="agreement",model="twoway")$value)
try(icc_f13_1 <- icc(acc[,grepl("f_acc_res",colnames(acc)) | grepl("f_t2newscore1",colnames(acc)) | grepl("f_t3newscore1",colnames(acc))],type="agreement",model="twoway")$value)
...
```{r speed_outliers, include=FALSE}
