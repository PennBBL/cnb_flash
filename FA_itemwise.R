...
colnames(noNA_sNL)[5:184] <- paste0("item_",1:180)
colnames(noNA_sN)[5:94] <- paste0("item_",1:90)


# making sure that there is a TTR for all CORR == 1
sNL_speed <- noNA_sNL
sN_speed <- noNA_sN
both <- intersect(unique(sNL_speed$test_sessions.datasetid),unique(sNL_acc$test_sessions.datasetid))
...
sNL_acc <- sNL_acc[sNL_acc$test_sessions.datasetid %in% both,]
both <- intersect(unique(sN_speed$test_sessions.datasetid),unique(sN_acc$test_sessions.datasetid))
sN_speed <- sN_speed[sN_speed$test_sessions.datasetid %in% both,]
sN_acc <- sN_acc[sN_acc$test_sessions.datasetid %in% both,]

sNL_speed <- sNL_speed[match(sNL_speed$test_sessions.datasetid,sNL_acc$test_sessions.datasetid),5:184]
sN_speed <- sN_speed[match(sN_speed$test_sessions.datasetid,sN_acc$test_sessions.datasetid),5:94]
sNL_acc <- sNL_acc[5:94]
sN_acc <- sN_acc[5:94]
...
for (i in 1:nrow(sN_speed)){
        spe <- sN_speed[i,]
        acc <- sN_acc[i,]
        sN_corr[i,] <- ifelse(acc==1, ifelse(!is.na(spe),1,0), ifelse(is.na(spe),1,NA))
}
sNL_corr <- sNL_speed
...
for (i in 1:nrow(sNL_speed)){
        spe <- sNL_speed[i,]
        acc <- sNL_acc[i,]
        sNL_corr[i,] <- ifelse(acc==1, ifelse(!is.na(spe),1,0), ifelse(is.na(spe),1,NA))
}


# not done with the part above, but I ended up finding that there are groups of items that everyone has the correct TTR and CORR relationship but only in weird intervals. (when corr=1, ttr!=NA, and corr=0, ttr!=NA)
...
sNL_meansd <- rbind(sNL_fmeansd,sNL_nmeansd)
...
spe1_means <- ggplot(sNL_meansd,aes(x=Item,y=MeanTTR,color=factor(Flash))) +
...
ggsave("SPCPTNL_means.png",height = 10,width = 30)


spe2_means <- ggplot(sN_meansd,aes(x=Item,y=MeanTTR,color=factor(Flash))) +
        geom_point(position=position_dodge(.9)) +
        geom_errorbar(aes(ymin=MeanTTR-sd, ymax=MeanTTR+sd), width=.2,position=position_dodge(.9)) +
        theme_bw() + ylab("Response Time (ms)") +
        scale_color_discrete(name = "Test Administration", labels = c("Non-Flash (n=133)", "Flash (n=145)")) +
...
ggsave("SPCPTN90_means.png",height = 10,width = 15)
...
noNA_sN[,5:94] <- sapply(noNA_sN[,5:94]+1,log)
...
log_spe1_means <- ggplot(sNL_meansd,aes(x=Item,y=MeanTTR,color=factor(Flash))) +
        geom_point(position=position_dodge(.9)) +
        geom_errorbar(aes(ymin=MeanTTR-sd, ymax=MeanTTR+sd), width=.2,position=position_dodge(.9)) +
...
ggsave("SPCPTNL_logmeans.png",height = 10,width = 30)
...
ggsave("SPCPTN90_logmeans.png",height = 10,width = 15)
